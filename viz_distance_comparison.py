...
def get_route_distances() -> Dict:
    ...
    try:
        ...
        return {'distances': [], 'avg_distance': 0, 'median_distance': 0, 'std_distance': 0}
...
def create_distance_comparison_chart(perceived_data: Dict, actual_data: Dict, linked_data: List[Dict]) -> go.Figure:
...
def link_perceived_and_actual_distances(df: pd.DataFrame, actual_data: Dict) -> List[Dict]:
    ...
    for idx, row in df.iterrows():
        ...
        if (pd.notna(submission_id) and 
            pd.notna(distance_importance_raw) and 
            submission_id in route_lookup):
...
def main():
    ...
    if perceived_data['responses']:
        print(f"   • Perceived importance responses: {perceived_data['response_count']}")
        print(f"   • Average importance rating: {perceived_data['avg_importance']:.2f}/5")
