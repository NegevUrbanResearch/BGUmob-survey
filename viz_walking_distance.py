...
def create_histogram(walking_df: pd.DataFrame, avg_km: float) -> go.Figure:
    """Create histogram of walking distances with mean line and annotation."""
    distances = walking_df['total_distance_km']
    ...
    fig.add_trace(
        go.Histogram(
            x=distances,
            nbinsx=20,
            marker=dict(color='rgba(0, 212, 255, 0.7)', line=dict(color='rgba(255,255,255,0.2)', width=1)),
    ...
    fig.add_shape(
        ...
        xref='x',
        yref='paper',
        line=dict(color='rgba(255, 107, 107, 0.9)', width=3, dash='dash')
    )
    ...
    fig.update_layout(
        ...
        bargap=0.08,
        hoverlabel=dict(bgcolor='rgba(15,15,15,0.98)', bordercolor='rgba(255,255,255,0.4)')
    )
...
def export_figure(fig: go.Figure, filename_base: str, title: str) -> None:
    ...
    try:
        fig.write_image(f'outputs/{filename_base}.png', width=1200, height=800, scale=2, engine='kaleido')
    except Exception:
        # HTML is the primary output; PNG is optional
        pass


def save_stats(avg_km: float) -> None:
    stats = {
...
def main() -> Tuple[pd.DataFrame, float]:
    ...
    fig = create_histogram(walking_df, avg_km)
